...
function [Results,AInfo]= ACD_NeuronCombinedClassifier_CPP_v16p0(AInfo,CD,DispInfoLevel,Options)
...
AInfo.UsableFeatures = CD.CD_GetUsableFeaturesIndex_v16p0();
AInfo.UsableFeatures(AInfo.UsableFeatures == 0) = [];
...
%             mdl.(fS) = fitcnb(Fnc.TrainingDataTmp.(fS), Fnc.TrainingClassTmp.(fS),'CategoricalPredictors',CurrentAttCatStat);
			TrainDecisionTree(Fnc.TrainingDataTmp.(fS), double(Fnc.TrainingClassTmp.(fS))+1,'c:\tempResults\tree.txt',5,1000);
% 					'HyperparameterOptimizationOptions',struct('ShowPlots',false,'Verbose',0,'UseParallel',true));%,'MaxDepth',);
			
			Out0 = RunDecisionTree(Fnc.TestingDataTmp.(fS),'c:\tempResults\tree.txt');
            Fnc.TestPredictions.(fS) = Out0+1;
% 			predict(mdl.(fS),
...
%     Results.Models(Record.Index).Model = mdl;
    ...
    AInfo = ACD_DisplayStatus_Global_v16p0(AInfo,Results,CD,DispInfoLevel,Options);
end
