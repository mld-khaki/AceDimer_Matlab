...
function [FeatureContributionsWithOrgOrder, OutFeatures] = ACD_ContributionEvaluation_IndividualClassBased_v16p0(ClassesData,FeaturesVector,Top_N_Percent,AccuraciesVector,ConfusionMatrices,AllPossibleCombinations,BypassAccuracyCheck,IgnoreBaseContribution,Options)
...
[CombinationLength, ~ ] = local_SizeLong(AccuraciesVector);

StartingPower = 2;
PowVal = StartingPower;
while(1)
	ContribAr = [];
	if PowVal > 12
	end
	...
	if Options.Forward1Reverse0 == 1
		for combinationCtr=1:CombinationLength
			...
			%     		continue;
			% 		end
			ConfusionMat = CombineConfusionMatrices(ConfusionMatrices(combinationCtr));
			ContributionValue = ACD_GetContributionFromCofusion_v16p0(ConfusionMat.^PowVal);
			ContribAr(end+1) = ContributionValue;
			if ContributionValue <= SortedIndFeaturesContrib.BaselineAccuracy
				continue;
			end
			...
			for featureCtr=1:length(Vector)
				...
				ContributionMain =(ContributionValue - SortedIndFeaturesContrib.BaselineAccuracy);
				ContributionMain = ContributionValue;
	...
	if		(nansum(ContribAr > 0) < 100 || ...
			nansum(IndividualFeaturesContributions > 0) == 0 || ...
			nansum(IndividualFeaturesContributions > 0) < nansum(ACD_ExtractStructField(FeaturesVector,'Enabled'))/2) && ... half of the previously available features 
			sum(~isnan(ContribAr)) ~= 0
		PowVal = PowVal + 1;
	elseif sum(~isnan(ContribAr)) == 0
		error('All Nans');
	else
		break;
	end
...
function OutMat = CombineConfusionMatrices(InpFolds)
Fields = fieldnames(InpFolds);
OutMat = zeros(size(InpFolds.(Fields{1})));
for iCtr=1:length(Fields)
	OutMat = OutMat + InpFolds.(Fields{iCtr});
end
end
