...
function [objOut,BNsFolds,BScFolds] = CD_BalanceFoldsAndScarceFolds_v3p0p0(obj,NsFolds,ScFolds,JitterWeight,JitterPercentage)
                ...
                if nansum(Folds(fCtr).ClassCnts == MaxClassCount) == length(ClassIndexVector) % either NonScarceClass or ScarceClass vector
                    ...
                    for mCtr=1:MaxClassCount - Folds(fCtr).ClassCnts(cCtr)
						...
						if JitterWeight > 0 && JitterPercentage > 0
							TempObsVector = ACD_AddJitterToGaussianData_v3p0p0(tmpFold,TempObsVector,JitterPercentage,JitterWeight);
						end
%                         for 
