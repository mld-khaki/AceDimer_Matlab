...
function AInfo = ACD_DisplayStatus_Global_Parfor_v16p0(AInfo,DispInfoLevel,Options,varargin,ProgressCtr)
...
if mod(ProgressCtr,AInfo.TicStart.TicCnt) == 0 || ProgressCtr == 0 || AInfo.BypassEverything == 1
	...
	if (~isempty(StringOut) && DispInfoLevel.TimingInfo == 1)
		...
		ACD_cprintf('blue','\n\tSubj=%s, Date=%s, Epoch=%s\n',Options.SubjectStr,Options.StartTime,Options.CurrentEpoch);
		fprintf('\n\t===================================');
		for sCtr=2:Options.CurAttributeCount
			...
			try
				if GotData == 1 && ~isinf(MaxVal)
					...
					ACD_cprintf('blue', '\n\tAvg Acc = %5.2f%%, for (%u) Attributes',ACD_nanmean(AInfo.Avgs{CurrentFeatureCount})*100,CurrentFeatureCount);
					PlotHappened = 1;
				end				
				if GotData == 1 
					ACD_cprintf('blue', '\n\tAvg Acc = %5.2f%%, for (%u) Attributes',ACD_nanmean(AInfo.Avgs{CurrentFeatureCount})*100,CurrentFeatureCount);
