...
classdef ClassificationData_v16p0
	...
	methods (Access = private)
		[obj,NsFolds,ScFolds]		= CD_GenerateFoldIndex_v16p0(obj,ObservationVector,ClassVector,FoldCount);
		[obj,BNsFolds,BScFolds]		= CD_BalanceFoldsAndScarceFolds_v16p0(obj,NsFolds,ScFolds,JitterWeight,JitterPercentage);
		%         Out = CD_Matrix_Fuse(obj,Input1,Input2,FuseType)
		[Folds, Classes, ClassIndex] = CD_GetTstTrnFold_v16p0(obj,NsFolds,ScFolds,TTrainFTest,FoldCount,SelectedFold);
		...
		function obj = CD_BalanceData(obj)
			if (obj.ForcedBalanced == 1)
				...
				[obj,obj.TrainFoldsBNs, obj.TrainFoldsBSc]	= obj.CD_BalanceFoldsAndScarceFolds_v16p0(obj.TrainFoldsNs,obj.TrainFoldsSc,obj.JitterWeight,obj.JitterPercentage);
				[obj,obj.TestFoldsBNs , obj.TestFoldsBSc]  = obj.CD_BalanceFoldsAndScarceFolds_v16p0(obj.TestFoldsNs, obj.TestFoldsSc,obj.JitterWeight,obj.JitterPercentage);
		...
		function obj = CD_InitializeVariablesWithInputData(obj)
			...
			FeatureIndex(obj.CD_GetUsableFeaturesIndex_v16p0() == 0) = 1;
			...
			[obj,obj.TrainFoldsNs,obj.TrainFoldsSc] = CD_GenerateFoldIndex_v16p0(obj,obj.TrainingObs,obj.TrainingCls,obj.FoldCount);
			[obj,obj.TestFoldsNs, obj.TestFoldsSc ] = CD_GenerateFoldIndex_v16p0(obj,obj.TestingObs, obj.TestingCls, obj.FoldCount);
		end
		...
		function FoldIndexes = CD_GetFoldIndexes_v16p0(obj,Count,FoldNum)
			FoldIndexes = {};
			LastNum = 1;
			for fCtr = 1:FoldNum
				if fCtr < FoldNum
	...
	methods (Access = public)
		function obj = ClassificationData_v16p0(Version,varargin)
		...
		function obj = CD_UpdateVariablesWithUsableFeatures_v16p0(obj,NewUsableFeatures)
...
% 					FeatureIndex
		...
		function obj = CD_UpdateVariablesWithNewVector_v16p0(obj,NewEnabledFeaturesVector)
			%disable all features
			%enable only the features that are pointed out by the
			%NewUsableFeatures variable
		...
		function OutObs = CD_GetTrainingObs_BOFoldNum_v16p0(obj,FoldNum)
			[OutObs,~,~] = CD_GetTstTrnFold_v16p0(obj,obj.TrainFoldsBNs,obj.TrainFoldsBSc,1,FoldNum);
		end
		
		function OutCls = CD_GetTrainingCls_BOFoldNum_v16p0(obj,FoldNum)
			[~,OutCls,~] = CD_GetTstTrnFold_v16p0(obj,obj.TrainFoldsBNs,obj.TrainFoldsBSc,1,FoldNum);
		end
		
		function OutInd = CD_GetTrainingInd_BOFoldNum_v16p0(obj,FoldNum)
			[~,~,OutInd] = CD_GetTstTrnFold_v16p0(obj,obj.TrainFoldsBNs,obj.TrainFoldsBSc,1,FoldNum);
		end
		
		% get testing observations based on Fold number
		function OutObs = CD_GetTestingObs_BOFoldNum_v16p0(obj,FoldNum)
			if obj.FoldCount > 1
				[OutObs,~] = CD_GetTstTrnFold_v16p0(obj,obj.TrainFoldsBNs,obj.TrainFoldsBSc,0,FoldNum);
			else
				[OutObs,~] = CD_GetTstTrnFold_v16p0(obj,obj.TrainFoldsBNs);
			end
		end
		
		function OutCls = CD_GetTestingCls_BOFoldNum_v16p0(obj,FoldNum)
			if obj.FoldCount > 1
				[~,OutCls,~] = CD_GetTstTrnFold_v16p0(obj,obj.TrainFoldsBNs,obj.TrainFoldsBSc,0,FoldNum);
			else
				[~,OutCls,~] = CD_GetTstTrnFold_v16p0(obj,obj.TrainFoldsBNs);
			end
		end
		
		function OutInd = CD_GetTestingInd_BOFoldNum_v16p0(obj,FoldNum)
			if obj.FoldCount > 1
				[~,~,OutInd] = CD_GetTstTrnFold_v16p0(obj,obj.TrainFoldsBNs,obj.TrainFoldsBSc,0,FoldNum);
			else
				[~,~,OutInd] = CD_GetTstTrnFold_v16p0(obj,obj.TrainFoldsBNs);
			end
		end
		
		function Out = CD_GetUsableFeaturesVector_v16p0(obj)
			Out = obj.Features;
		end
		
		function Out = CD_GetUsableFeaturesIndex_v16p0(obj)
			Out = [];
			for iCtr=1:length(obj.Features)
		...
		function obj = CD_DisableFeature_v16p0(obj,FeatureNumber)
			obj.Features(FeatureNumber).Enabled = 0;
		end
		
		function obj = CD_EnableFeature_v16p0(obj,FeatureNumber)
			obj.Features(FeatureNumber).Enabled = 1;
		end
		
		function State = CD_GetFeatureEnabledStatus_v16p0(obj,FeatureNumber)
