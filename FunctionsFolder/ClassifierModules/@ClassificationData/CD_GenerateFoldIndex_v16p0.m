...
function [obj,NsFolds,ScFolds] = CD_GenerateFoldIndex_v16p0(obj,ObservationVector,ClassVector,FoldCount)
obj.MetaData = [];
...
% **************************** main function end *****************************
    ...
    function [AvailableData] = GenerateIndexOfData(Data)
        ...
        for dCtr=1:length(Data.ClassVector)
            ...
            if sum(ACD_RowColVectEqual_v16p0(Data.ScarceClsInds,AvailableData(dCtr).ClassIndex)) >= 1
        ...
        for dCtr=1:length(AvailableData)
            if (AvailableData(dCtr).Scarce ~= ScarceAccepted) || (AvailableData(dCtr).AssignedFold ~= -1)
				...
				if isempty(CurrentClassMainIndex)
					...
					CurrentClassIndex = find(ACD_RowColVectEqual_v16p0(CurrentClassMainIndex,CorrespondingClasses));
