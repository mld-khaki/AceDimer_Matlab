function [FeaturesSt,HistCheckData] = ACD_FeatureCombinations_Analyzer__Step1_v3p3p0(InpObs,InpClasses,RemainingFeatures,Verbose,FeaturesSt,ModeStr,ClsType)
...
while(DoneFinding <= 2)
    ...
    if length(CurFeatureSet)>=1 
        ...
        if SelSetCount ~= length(CurFeatureSet)
            ...
            [IndicesMRMRMinimal,~] = Local_FS(InpObs(:,CurFeatureSet),InpClasses,'Verbose',1);
            ...
            Params.Repetitions = RepCount;
            Params.Classifier = ClsType;
            Params.CurFeatureSet = SelSet;
            [CurAccMinimal,~,~,HistCheckData] = ACD_EvalAcc_v3p3p0(HistCheckData,InpObs,InpClasses,Params);
        end
        Params.Repetitions = RepCount;
        Params.Classifier = ClsType;
        Params.CurFeatureSet = [SelSet MaxInd];
        [NewAccMinimal,~,~,HistCheckData] = ACD_EvalAcc_v3p3p0(HistCheckData,InpObs,InpClasses,Params);
    else
        NewAccMinimal = inf;
    end
