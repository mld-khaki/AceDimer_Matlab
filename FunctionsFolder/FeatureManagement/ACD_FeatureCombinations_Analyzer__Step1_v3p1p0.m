function FeaturesSt = ACD_FeatureCombinations_Analyzer__Step1_v3p1p0(InpObs,InpClasses,RemainingFeatures,Verbose,FeaturesSt,ModeStr)
...
[CurAcc,CurStd,Conf] = ACD_EvalAcc(InpObs(:,CurFeatureSet),InpClasses);
CurAccMinimal = -inf;
...
%         OrgAvailableFeatures = OrgAvailableFeatures(IndicesMRMR);
        ...
        while(~isempty(IndicesBadImpact) && length(CurFeatureSet) >= 2)
            ...
            [tmpAcc,tmpStd] = ACD_EvalAcc(InpObs(:,tmpFeatureSet),InpClasses);
