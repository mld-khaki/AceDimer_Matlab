...
for fCtr = 1:InpRes.ClassesData.GetFoldCount()
    ...
    elseif strcmpi(class(InpRes.ClassesData),'ClassificationData_v2p0p0') == 1
        ...
        error('Unknown version of the class <ClassificationData>!!');
    end
end
...
AllPossSortedCombinations = ACD_Contribution_Lookup_Preparation_v2p1p0(InpResults(end-1).AnalysisInfo.AllPossibleCombinations);
ACD_Contribution_Lookup_Prv_Accuracy_v2p1p0(AllPossSortedCombinations,[],true);
...
for combinationCtr=1:CombinationLength
    ...
    for featureCtr=1:length(Vector)
        PrvVect = Vector;
        PrvVect(featureCtr) = [];
        PrvInd = ACD_Contribution_Lookup_Prv_Accuracy_v2p1p0(AllPossSortedCombinations,PrvVect,false);
