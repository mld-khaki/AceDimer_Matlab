...
function [AnalysisResults] = ACD_AUX_CalculateContributions_v16p0(Version,InpRes,BypassAccuracyCheck,IgnoreBaseContribution,Top_N_Percent)
...
IndividualFeaturesContributions = zeros(1,length(InpRes.Features));
MaxividualFeaturesContributions = zeros(1,length(InpRes.Features));
IndividualFeaturesCount = zeros(1,length(InpRes.Features));
...
for combinationCtr=1:CombinationLength
	...
	for featureCtr=1:length(Vector)
        ...
        ConfScore = ACD_AUX_ConfusionMatrixScore_v16p0(InpRes.ConfusionMatrices(combinationCtr));
