...
function LDA_Model = ACD_LDA_v16p0(Input,Target,Priors)
...
LDA_Model  = NaN(ObservationCount,m+1);   % model coefficients
...
for iCtr = 1:ObservationCount
    ...
    % Constant
    LDA_Model(iCtr,1) = -0.5 * Temp * GroupMean(iCtr,:)' + log(PriorProb(iCtr));
    
    % Linear
    LDA_Model(iCtr,2:end) = Temp;
end
