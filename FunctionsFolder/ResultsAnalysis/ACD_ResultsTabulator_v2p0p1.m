...
function [AnalysisOut, SROut,Rankings] = ACD_ResultsTabulator_v2p0p1(Version,InputFolderOfResults,Top_N_Percent,SRIn,AnalysisInp)
CurrentVersion = 'AceDimerV2p0p1';
...
AllSRs = SR(1).SR.SaveResults;
for fCtr=2:length(SR)
    AllSRs(fCtr) = SR(fCtr).SR.SaveResults;
end
...
% 	end
    ...
    ContAnalysisRes = ACD_AUX_CalculateContributions_v2p0p0('AceDimerV2p0p0',...
		AllSRs,...
